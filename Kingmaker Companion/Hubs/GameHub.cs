...
public class GameHub : Hub<IGameClient> {
    ...
    public async Task MapUpdate(string message) {
        ...
        JObject k = JObject.Parse(redis.GetDatabase().StringGet("kingdom:" + g.KingdomId));
        JArray h = (JArray) k["saveHexes"];
        
        h[Int32.Parse(m["index"].ToString())][m["property"].ToString()] = m["newState"].ToString();

        string s = Regex.Replace(k.ToString(), @"\s(?=([^""]*""[^""]*"")*[^""]*$)", string.Empty);
    ...
    private async Task<string> newKingdom() {
